# claude-mgr 初始设计与 MVP 规格（已归档）

> **DEPRECATED / 已归档。** 本文是项目最初作为 `README.md` 编写的设计与 MVP 规格说明，
> 记录了立项时的目标、边界、模块设计与验收标准，作为历史决策记录保留。
> 它**不再**作为项目入口文档维护，部分细节可能已与当前实现产生偏差。
>
> - 项目入口与上手说明见根目录 [`../README.md`](../README.md)。
> - 面向编码 agent 的操作约定见 [`../AGENTS.md`](../AGENTS.md)，记录的是当前有效的工程规则。
> - 当前协议事实与运行命令以 `AGENTS.md`、代码和测试为准；本文与之冲突时以前者为准。
